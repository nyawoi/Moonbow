...
{
    internal static class MuteUserPatch
    {
        private const string HarmonyPatchId = "net.aethar.Moonbow.Patches.MuteUserPatch";
        private static IEnumerable<IServerConnectionHook> ConnectionHooks => ConnectionComponentHook.ServerConnectionHooks;

        private static void Postfix(bool __result, string uid, int minutes)
        {
            if (!__result) return;
            ...
            foreach (var hook in ConnectionHooks)
            {
                try
                {
                    ...
                    {
                        hook.OnPlayerMuted(playerEntity, minutes);
                    }
                }
                catch (Exception e)
                {
        ...
        public static void Initialize()
        {
            ...
            var methodBase = AccessTools.Method(typeof(RightsManager), nameof(RightsManager.MuteUser));
            var methodPostfix = AccessTools.Method(typeof(MuteUserPatch), nameof(Postfix));
